...
/* The maker row above the display cards. Quieter than a chip: it is a way
   of narrowing a list, not an answer being given, and it should not compete
   with the cards it filters. */
.brands { display: flex; flex-wrap: wrap; gap: 7px; margin: 0 0 14px; }
.brand {
  font: inherit; font-size: var(--t-sm); cursor: pointer;
  padding: 8px 13px; min-height: 38px; display: inline-flex;
  align-items: center; gap: 7px;
  border: 1px solid var(--line); border-radius: 99px;
  background: var(--surface); color: var(--ink-2);
}
.brand:hover { border-color: var(--accent-line); color: var(--accent); }
.brand.on {
  border-color: var(--accent); color: var(--accent); font-weight: 620;
  background: var(--accent-soft);
}
.brand .n {
  font-family: var(--mono); font-size: var(--t-eyebrow); color: var(--ink-3);
}
.brand.on .n { color: var(--accent); }
...
/* The closing card. It prints: this is the one block somebody carries out
   to the machine, so it keeps its border and avoids being split across a
   page break. */
.finish {
  margin-top: 28px; padding: 18px 19px 19px;
  border: 1px solid var(--line); border-radius: 12px;
  background: var(--surface-2);
}
.finish h4 { margin: 0 0 2px; font-size: var(--t-lg); }
.finish > .fhead > div > p {
  margin: 0 0 12px; color: var(--ink-3); font-size: var(--t-sm);
}
.fhead { display: flex; gap: 16px; align-items: flex-start; }
.fmon {
  width: 108px; flex: none; aspect-ratio: 380/300; object-fit: contain;
  border-radius: 8px; background: var(--surface);
  border: 1px solid var(--line-soft);
}
.ffacts { display: grid; gap: 9px; margin: 0; }
.ffacts dt {
  font-size: var(--t-eyebrow); text-transform: uppercase;
  letter-spacing: .08em; color: var(--ink-3); font-weight: 700;
}
.ffacts dd {
  margin: 2px 0 0; font-weight: 600; font-size: var(--t-md);
  overflow-wrap: anywhere;
}
.ffacts code { font-family: var(--mono); font-size: var(--t-sm); }
.flead {
  margin: 16px 0 10px; font-size: var(--t-sm); color: var(--ink-2);
  padding-top: 14px; border-top: 1px solid var(--line);
}
/* Thumb size, not cap size: at this scale the glyph is recognisable across
   a cab rather than merely present. */
.ficons {
  display: grid; grid-template-columns: repeat(auto-fill, minmax(96px, 1fr));
  gap: 12px;
}
.ficons figure { margin: 0; text-align: center; }
.ficons img {
  width: 46px; height: 46px; border-radius: 7px; background: #f4f5f1;
  border: 1px solid var(--line-soft); display: block; margin: 0 auto 5px;
}
.ficons figcaption {
  font-size: var(--t-xs); color: var(--ink-2); line-height: 1.3;
}
...
@media print {
  .finish { break-inside: avoid; background: #fff; }
}
...
const S = { equip: null, brand: null, mon: null, ver: null, job: null,
            route: null };
...
function reset(from) {
  ...
  /* Brand is not in ORDER -- it filters, it does not answer. But going back
     to the machine means going back to the beginning, so it clears there. */
  if (from === 'equip') S.brand = null;
  render();
}
...
function finishPanel(p, m) {
  const wrap = el('div', { class: 'finish' });

  const facts = [];
  if (p.fmt && !/^n\/a/i.test(p.fmt)) facts.push(['File format', p.fmt, false]);
  if (p.path && !/^n\/a/i.test(p.path)) facts.push(['Exactly where it goes', p.path, true]);
  /* The stick's filesystem is a fact about a stick. On the typed route
     there is no stick, and printing FAT32 on a card for somebody who is
     about to key in four numbers is an instruction to do nothing. */
  if (p.t === 'usb' && p.fs && !/^n\/a/i.test(p.fs))
    facts.push(['Format the stick as', p.fs, false]);

  wrap.append(el('div', { class: 'fhead' },
    el('img', { class: 'fmon', src: D.icons[m.icon], alt: '' }),
    el('div', {},
      el('h4', {}, 'Take this to the machine'),
      el('p', {}, m.label),
      el('dl', { class: 'ffacts' }, facts.map(([k, v, code]) =>
        el('div', {}, el('dt', {}, k),
           el('dd', {}, code ? el('code', {}, v) : v)))))));

  /* Every application icon this procedure actually names, in the order the
     steps name them -- not the display's whole icon set, which would be a
     catalogue nobody reads. */
  const table = D.screenIcons[S.mon] || {};
  const seen = new Set();
  const used = [];
  p.steps.forEach(step => {
    const marks = step.match(/«([^»]*)»/g) || [];
    marks.forEach(raw => {
      const label = raw.slice(1, -1);
      const icon = table[label.toLowerCase()];
      if (icon && !seen.has(label)) { seen.add(label); used.push([label, icon]); }
    });
  });
  if (used.length) {
    wrap.append(el('p', { class: 'flead' },
      used.length === 1 ? 'The icon to look for on the display:'
                        : 'The icons to look for on the display:'));
    wrap.append(el('div', { class: 'ficons' }, used.map(([label, icon]) =>
      el('figure', {},
        el('img', { src: icon.src, alt: '', title: icon.code }),
        el('figcaption', {}, label)))));
  }
  return wrap;
}
...
/* A tractor fits twenty-two displays from sixteen makers, and a wall of
   twenty-two cards is not a choice, it is a search. The brand row cuts it to
   two or three in one press.

   A filter rather than a sixth question: the brand is not another fact the
   guide needs -- it already knows every display's maker -- it is a way of
   not reading. So it never blocks, "All makes" stays selected by default,
   and somebody who does not know the brand simply scrolls as before. */
function drawMonitors() {
  show('#q2', answered('equip') && !answered('mon'));
  if (!answered('equip') || answered('mon')) return;

  const fits = D.monitors.filter(m => m.equipment.includes(S.equip));
  const brands = [...new Set(fits.map(m => m.brand))].sort();
  if (!brands.includes(S.brand)) S.brand = null;

  const bar = $('#brands'); bar.replaceChildren();
  /* One maker and the row is noise: it would filter nothing. */
  if (brands.length > 1) {
    const pick = b => { S.brand = b; drawMonitors(); };
    bar.append(el('button', {
      class: 'brand' + (S.brand ? '' : ' on'), type: 'button',
      onclick: () => pick(null),
    }, 'All makes', el('span', { class: 'n' }, fits.length)));
    brands.forEach(b => bar.append(el('button', {
      class: 'brand' + (S.brand === b ? ' on' : ''), type: 'button',
      onclick: () => pick(b),
    }, b, el('span', { class: 'n' },
             fits.filter(m => m.brand === b).length))));
  }

  const host = $('#mons'); host.replaceChildren();
  fits.filter(m => !S.brand || m.brand === S.brand).forEach(m => {
    host.append(el('button', {
...
function showResult(t) {
  ...
  /* The last thing before the buttons, and the thing that goes to the
     machine. Everything above is read sitting down; this is the card you
     glance at with the stick in your hand -- the display you are walking to,
     the two or three facts that decide whether it works, and the actual
     glyphs you will be hunting for on the glass.

     The icons are not decoration. A step says «File Manager» and the eye has
     to find that picture among twenty on a home screen; printed at cap size
     inside a sentence they are a hint, printed here at thumb size with their
     names they are a key. Only the displays whose manufacturer publishes an
